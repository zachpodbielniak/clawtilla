...
container_start(ClawtComputer *computer, GError **error)
{
    ...
    /*
     * Ask before replaying.  Without this a start found the state it
     * wrote last time, skipped provisioning, and asked podman to start a
     * container that no longer existed -- so the one command an operator
     * reaches for to recover was the one that could not.
     *
     * A backend that cannot be asked is not a reason to refuse the start:
     * the start itself is about to fail with a better message than
     * anything that could be said here.
     */
    if (!container_reconcile(computer, &stale))
        g_debug("container %s: could not be reconciled: %s", self->name,
                stale->message);
