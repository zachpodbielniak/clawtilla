...
clawt_web_stylesheet(void)
{
    ...
    /* Native theme tokens and restrained spacing keep navigation readable
     * in every palette, including user overrides and keyboard-only use. */
    ":focus-visible{outline:2px solid var(--info-fg);outline-offset:3px}"
    ".navigation-launch{margin:12px 12px 4px;padding:10px 12px;"
      "border:1px solid var(--line-strong);border-radius:8px;"
      "color:var(--ink-2);text-decoration:none;font-weight:500}"
    ".navigation-launch:hover{background:var(--surface-2)}"
    ".navigation-page{max-width:42rem;margin:auto;padding:24px 20px 48px}"
    ".navigation-page h2.section{font-size:28px;margin:24px 0 8px;letter-spacing:-.02em}"
    ".navigation-search{display:flex;gap:8px;margin:24px 0 16px}"
    ".navigation-search input{flex:1;min-width:0}"
    ".navigation-results{border:1px solid var(--line);border-radius:12px;overflow:hidden}"
    ".navigation-result{display:block;padding:14px 18px;text-decoration:none;color:var(--ink)}"
    ".navigation-result+.navigation-result{border-top:1px solid var(--line)}"
    ".navigation-result:hover,.navigation-result:focus-visible{background:var(--surface-2)}"
    ".navigation-result:focus-visible{outline-offset:-3px}"
    ".navigation-result p{margin:0;overflow-wrap:anywhere}"
    ".navigation-result .navigation-name{font-weight:600;margin-bottom:4px}"
    ".navigation-count{font-variant-numeric:tabular-nums;font-size:13px}"
    ...
    ".team-name{font-size:12px;letter-spacing:0.02em;"
      "font-weight:600}"
    ...
    "@media (max-width:56rem){"
      ...
      /* Keep the real checkbox keyboard-operable without JavaScript.
       * Absolute positioning keeps it out of the two-track grid. */
      ".nav-toggle{display:block;position:absolute;width:1px;height:1px;"
        "padding:0;border:0;overflow:hidden;clip-path:inset(50%)}"
      ".nav-toggle:focus-visible~.content .nav-button{"
        "outline:2px solid var(--info-fg);outline-offset:3px}"
